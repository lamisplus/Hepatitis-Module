...
import { useValidateExistingPatientRegistrationFormValuesHook } from "../../ExisitingPatientComponents/Forms/FormvalidationSchemas/useValidateExistingPatientRegistrationFormValues.hook";
...
import { updateEnrolment } from "../../../../services/updateEnrolment";
import { id } from "date-fns/locale";
import { FETCH_ENROLMENT_KEY } from "../../../../utils/queryKeys";
import { fetchEnrolment } from "../../../../services/fetchEnrolment";
import { fetchFullPatientDetail } from "../../../../services/fetchPatientFullDetail";
...
const PatientDashboardEnrolmentViewUpdate = ({
  patientObj,
  setActiveContent,
  activeContent,
  disableInputs = false,
  ...
  const facilityId = patientObj?.facilityId;
  const [recordId] = useState(activeContent?.record?.recordId);
  const [formInitialValue, setFormInitialValue] = useState(null);
  const [patientFullDetails, setPatientFullDetails] = useState(patientObj);
  const patientId = patientFullDetails?.id;
  const patientGenderId = patientFullDetails?.gender?.id;
  ...
  const { mutate, isLoading } = useMutation({
    ...
    onSuccess: (data) => {
      toast.success("Enrolment update successfully");
      setActiveContent((prev) => ({ ...prev, route: "recent-history" }));
    },
    onError: () => {
      toast.error("Enrolment update failed");
    },
  });
  ...
  useQuery(
    ...
    () => fetchEnrolment(patientObj?.personUuid),
    {
      onSuccess: (data) => {
        const initialValues = {
          ...data,
          pregnancy: data?.screening?.pregnancy,
          hepatitisC: data?.screening?.hepatitisC,
          dateOfFirstHepatitisBPositiveScreening: formatDate(
            data?.screening?.dateOfFirstHepatitisBPositiveScreening
          ),
        };
  ...
  useQuery(
    ["FETCH_PATIENT_FULL_DETAILS_KEY", patientObj?.id],
    () => fetchFullPatientDetail(patientObj?.id),
    {
      onSuccess: (data) => {
        setPatientFullDetails(data);
      },
      refetchOnMount: "always",
    }
  );
  ...
  const handleSubmit = (values) => {
    ...
    mutate({ id: recordId, data: payloadData });
  };

  const { returnData: careEntryPointCodeset } =
    useFetchCodesets(careEntryPointPath);

  const { formik } = useValidateExistingPatientRegistrationFormValuesHook(
    handleSubmit,
    patientGenderId
  );
...
export default PatientDashboardEnrolmentViewUpdate;
